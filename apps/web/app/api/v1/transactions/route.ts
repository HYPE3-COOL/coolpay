...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    if (query.has('id')) filter.id = BigInt(query.get('id')!);
    if (query.has('hash')) filter.hash = query.get('hash')!;
    if (query.has('type')) filter.type = query.get('type')!;
    if (query.has('user_id')) filter.user_id = BigInt(query.get('user_id')!);
    if (query.has('from_address')) filter.from_address = query.get('from_address')!;
    if (query.has('to_address')) filter.to_address = query.get('to_address')!;
    if (query.has('status')) filter.status = query.get('status')!;
